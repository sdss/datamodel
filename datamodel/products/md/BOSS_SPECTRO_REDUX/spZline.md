...
### Naming Convention
`spZline-15000-59216\.fits`, Replace this filename with a regular expression that matches the file name.  For example, a six-digit number would correspond to `[0-9]{6}`.  `template-123456.fits` would be `template-[0-9]{6}\.fits`
...
| RUN1D | master | Spectro-1D reduction name |
| VERS1D |  | Version of idlspec2d for 1D reduction |
| VERSLINE |  | Version of idlspec2d for line fitting |
