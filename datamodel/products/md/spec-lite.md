...
## Basic Information
The spec-field-MJD-catalogid.fits files contain a repackaging of all spectral data for a given target coadd, including both the coadded spectrum. This groups the information from spFrame, spCFrame, spFlat, spPlate, spZbest, spZline, and spAll so that for each object one only needs to read a single file.
