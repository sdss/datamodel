...
class Page(object):
    ...
    def __init__(self, options=None, tree_ver=None, space_ver=None, file_spec=None, path=None,
                 env_label=None, location=None, force=None, verbose=None, debug=None):
        self.tree_ver = options.tree_ver if options else tree_ver
        self.space_ver = options.space_ver if options else space_ver

        if self.space_ver is None:
            self.space_ver = "sdsswork"
        ...
        if self.path is None:
            ...
            self.path = (
                join(self.env_label, self.location) if self.env_label and self.location else None
            )
        else:
            try:
    ...
    def set_path_from_access(self, access=None):
        ...
        if access_path and exists(access_path):
            with open(access_path) as file:
                if self.verbose:
                    print("PAGE> access %s" % access_path)
                self.access = load(file, Loader=FullLoader)
                _file_spec, _path = self.access.split(" = $", 2)
                if self.path is None:
                    self.path = _path
                    if self.verbose:
                        print("PAGE> path=%r" % self.path)
                elif self.path != _path:
                    print(
                        "PAGE> Aborting due to conflict in path (does not match) access file definition"
                    )
                    self.path = None
    ...
    def create_parent(self):
        if self.remote.pagelist:
            if self.verbose:
                print("PAGE> found %s page (version=%r)" % (self.env_label, self.space_ver))
        else:
    ...
    def create_datamodel(self):
        if self.remote.pagelist:
            ...
            if page:
                if self.verbose:
                    print("PAGE> found %s page (version=%r)" % (self.file_spec, self.space_ver))
            else:
            ...
            print(
                "PAGE> error creating %s page without finding parent %s page (version=%r)."
                % (self.file_spec, self.env_label, self.space_ver)
            )
    ...
    def set_abstract_path(self):
        ...
        if self.verbose:
            print("PAGE> abstract_path=%r" % self.abstract_path)
