...
def add_repr(schema: Dict[str, Any], model: Type[BaseModel]) -> None:
    ...
    for key, mod in model.model_fields.items():
        if mod.repr is False:
...
class CoreModel(BaseModel):
    ...
    def __repr_args__(self):
        ...
        for i in super().__repr_args__():
            ...
            ext = self.model_fields[i[0]].json_schema_extra
            if ext and 'repr_attr' in ext:
...
class BaseList(BaseModel):
    ...
    def __getitem__(self, item: Union[str, int]) -> BaseModel:
        ...
        if isinstance(item, str) and item in self:
            vals = [i.name.lower() for i in self.root]
            return self.root[vals.index(item.lower())]
        elif isinstance(item, BaseModel):
            return self[self.root.index(item)]
    ...
    def sort(self, field: str, key: Callable = None, **kwargs) -> None:
        ...
        ``root`` attribute, to preserve the original BaseList object
        ...
        vals = sorted(self.root.copy(), key=key, **kwargs)
        self.root = vals
